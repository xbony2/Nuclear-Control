...
public class PacketDispSettingsAll implements IMessage,
	...
	public PacketDispSettingsAll(int x, int y, int z,
			Map<Byte, Map<UUID, Integer>> settings) {
		this.x = x;
		this.y = y;
		this.z = z;
	...
	public IMessage onMessage(PacketDispSettingsAll message, MessageContext ctx) {
		...
		for (Map.Entry<Byte, Map<UUID, Integer>> slotData : message.settings
				.entrySet()) {
			Map<UUID, Integer> setting = panel
					.getDisplaySettingsForSlot(slotData.getKey());
			for (Map.Entry<UUID, Integer> item : slotData.getValue().entrySet()) {
				UUID key = item.getKey();
				if (key == null) {
					continue;
				...
				setting.put(
						new UUID(key.getMostSignificantBits(), key
								.getLeastSignificantBits()), item.getValue());
			}
		}
		panel.resetCardData();
		return null;
