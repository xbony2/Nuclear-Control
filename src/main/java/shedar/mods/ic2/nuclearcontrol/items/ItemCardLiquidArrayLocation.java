...
public class ItemCardLiquidArrayLocation extends ItemCardBase
{
				...
				if (Math.abs(dx) <= range && Math.abs(dy) <= range && Math.abs(dz) <= range)
				{
					...
					if (storage != null)
					{
						if (storage.fluid != null)
						{
							...
							card.setInt(String.format("_%damount", i), (int)storage.fluid.amount);
							...
							if (liquidId == 0)
								card.setString(String.format("_%dname", i), LanguageHelper.translate("msg.nc.None"));
							else
								card.setString(String.format("_%dname", i), FluidRegistry.getFluidName(liquidId));							
						}
						card.setInt(String.format("_%dcapacity", i), (int)storage.capacity);
						foundAny = true;
					}
					else
					{
	...
	public List<PanelString> getStringData(int displaySettings, ICardWrapper card, boolean showLabels)
	{
			...
			if (showEach)
			{
				...
				else
				{
					if (showName)
					{
						line = new PanelString();
						if(showLabels)
							line.textLeft = StringUtils.getFormattedKey("msg.nc.InfoPanelLiquidNameN", i+1, card.getString(String.format("_%dname",i)));
						else
					...
					if (showAmount)
					{
						line = new PanelString();
						if(showLabels)
							line.textLeft = StringUtils.getFormattedKey("msg.nc.InfoPanelLiquidN", i+1, StringUtils.getFormatted("", amount, false));
						else
							line.textLeft = StringUtils.getFormatted("", amount, false);
						result.add(line);
					}
					if (showFree)
					{
						line = new PanelString();
						if (showLabels)
							line.textLeft = StringUtils.getFormattedKey("msg.nc.InfoPanelLiquidFreeN", i+1, StringUtils.getFormatted("", capacity - amount, false));
						else
					...
					if (showCapacity)
					{
						line = new PanelString();
						if(showLabels)
							line.textLeft = StringUtils.getFormattedKey("msg.nc.InfoPanelLiquidCapacityN", i+1, StringUtils.getFormatted("", capacity, false));
						else
					...
					if (showPercentage)
					{
						line = new PanelString();
						if (showLabels)
							line.textLeft = StringUtils.getFormattedKey("msg.nc.InfoPanelLiquidPercentageN", i+1, StringUtils.getFormatted("", capacity==0? 100:(((double)amount)*100L/capacity), false));
						else
							line.textLeft = StringUtils.getFormatted("", capacity==0? 100:(((double)amount)*100L/capacity), false);
						result.add(line);
					}
				}
			}
		...
		if (showSummary)
		{
			...
			{
				line = new PanelString();
				line.textLeft =  StringUtils.getFormatted("msg.nc.InfoPanelLiquidPercentage", totalCapacity==0? 100:(totalAmount*100/totalCapacity), showLabels);
				result.add(line);
			}
		}
		return result;
	...
	public List<PanelSetting> getSettingsList()
	{
		...
		result.add(new PanelSetting(LanguageHelper.translate("msg.nc.cbInfoPanelLiquidName"), DISPLAY_NAME, CARD_TYPE));
		//result.add(new PanelSetting(LanguageHelper.translate("msg.nc.cbInfoPanelLiquidAmount"), DISPLAY_AMOUNT, CARD_TYPE));
		result.add(new PanelSetting(LanguageHelper.translate("msg.nc.cbInfoPanelLiquidFree"), DISPLAY_FREE, CARD_TYPE));
		result.add(new PanelSetting(LanguageHelper.translate("msg.nc.cbInfoPanelLiquidCapacity"), DISPLAY_CAPACITY, CARD_TYPE));
		result.add(new PanelSetting(LanguageHelper.translate("msg.nc.cbInfoPanelLiquidPercentage"), DISPLAY_PERCENTAGE, CARD_TYPE));
		result.add(new PanelSetting(LanguageHelper.translate("msg.nc.cbInfoPanelLiquidEach"), DISPLAY_EACH, CARD_TYPE));
		result.add(new PanelSetting(LanguageHelper.translate("msg.nc.cbInfoPanelLiquidTotal"), DISPLAY_TOTAL, CARD_TYPE));		
		return result;
	}
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player, List info, boolean advanced) 
	{
		...
		if (cardCount > 0)
		{
			...
			String hint = String.format(LanguageHelper.translate("msg.nc.LiquidCardQuantity"), cardCount);
			info.add(hint);
		}
	}
}
