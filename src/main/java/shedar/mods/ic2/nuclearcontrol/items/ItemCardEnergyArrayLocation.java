			...
			for (int i = 0; i < cardCount; i++) {
				...
				if (Math.abs(dx) <= range && Math.abs(dy) <= range
					...
					EnergyStorageData storage = EnergyStorageHelper
							.getStorageAt(panel.getWorldObj(), coordinates[0],
									coordinates[1], coordinates[2], card
											.getInt(String.format(
													"_%dtargetType", i)));
					if (storage != null) {
						totalEnergy += storage.stored;
						card.setInt(String.format("_%denergy", i),
								(int) storage.stored);
						card.setInt(String.format("_%dmaxStorage", i),
	...
	public List<PanelString> getStringData(int displaySettings,
			...
			if (showEach) {
				if (isOutOfRange) {
					...
					line.textLeft = StringUtils.getFormattedKey(
							"msg.nc.InfoPanelOutOfRangeN", i + 1);
					result.add(line);
				} else if (isNotFound) {
					line = new PanelString();
					line.textLeft = StringUtils.getFormattedKey(
							"msg.nc.InfoPanelNotFoundN", i + 1);
					result.add(line);
				} else {
					if (showEnergy) {
						line = new PanelString();
						if (showLabels)
							line.textLeft = StringUtils
									.getFormattedKey("msg.nc.InfoPanelEnergyN",
					...
					if (showFree) {
						...
						if (showLabels)
							line.textLeft = StringUtils.getFormattedKey(
									...
									i + 1,
									StringUtils.getFormatted("", storage
											- energy, false));
						else
							line.textLeft = StringUtils.getFormatted("",
					...
					if (showStorage) {
						...
						if (showLabels)
							line.textLeft = StringUtils.getFormattedKey(
									"msg.nc.InfoPanelEnergyStorageN", i + 1,
									StringUtils
											.getFormatted("", storage, false));
						else
							line.textLeft = StringUtils.getFormatted("",
									storage, false);
						result.add(line);
		...
		if (showSummary) {
			if (showEnergy) {
				line = new PanelString();
				line.textLeft = StringUtils.getFormatted(
						"msg.nc.InfoPanelEnergy", totalEnergy, showLabels);
				result.add(line);
			}
			if (showFree) {
				line = new PanelString();
				line.textLeft = StringUtils.getFormatted(
						"msg.nc.InfoPanelEnergyFree", totalStorage
								- totalEnergy, showLabels);
				result.add(line);
			}
			if (showStorage) {
				line = new PanelString();
				line.textLeft = StringUtils.getFormatted(
						"msg.nc.InfoPanelEnergyStorage", totalStorage,
						showLabels);
				result.add(line);
			}
			if (showPercentage) {
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player,
		...
		if (cardCount > 0) {
			...
			String hint = String.format(
					LanguageHelper.translate("msg.nc.EnergyCardQuantity"),
					cardCount);
			info.add(hint);
		}
	}
