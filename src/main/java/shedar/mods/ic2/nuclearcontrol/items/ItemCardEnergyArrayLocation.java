			...
			if (showEach)
			{
				...
				else
				{
					...
					if (showPercentage)
					{
						line = new PanelString();
						if (showLabels)
							line.textLeft = StringUtils.getFormattedKey("msg.nc.InfoPanelEnergyPercentageN", i+1, StringUtils.getFormatted("", storage==0? 100:(((double)energy)*100L/storage), false));
						else
							line.textLeft = StringUtils.getFormatted("", storage==0? 100:(((double)energy)*100L/storage), false);
						result.add(line);
					}
				}
			}
