...
public class GuiPanelSlope extends GuiScreen {
	...
	public GuiPanelSlope(GuiInfoPanel parentGui,
			TileEntityAdvancedInfoPanel panel) {
	...
	protected void mouseClicked(int x, int y, int par3) {
		...
		if (y >= 23 && y <= 89) {
			...
			// NetworkHelper.initiateClientTileEntityEvent(panel,
			// offset+amount);
		}
	}
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		drawTexturedModalRect(left + 79, top + 25
				+ (panel.rotateHor < 0 ? 32 + panel.rotateHor * 4 / 7 : 32),
				186, 0, 14, Math.abs(panel.rotateHor * 4 / 7));
		drawTexturedModalRect(left + 137, top + 25
				+ (panel.rotateVert < 0 ? 32 + panel.rotateVert * 4 / 7 : 32),
				186, 0, 14, Math.abs(panel.rotateVert * 4 / 7));
