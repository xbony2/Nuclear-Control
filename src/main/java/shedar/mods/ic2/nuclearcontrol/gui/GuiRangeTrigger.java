...
public class GuiRangeTrigger extends GuiContainer {
	...
	private void initControls() {
		...
		for (int i = 0; i < 10; i++) {
			buttonList.add(new CompactButton(100 + i * 10, guiLeft + 30 + i
					* 12 + (i + 2) / 3 * 6, guiTop + 57, 12, 12, "-"));
			buttonList.add(new CompactButton(100 + i * 10 + 1, guiLeft + 30 + i
					* 12 + (i + 2) / 3 * 6, guiTop + 79, 12, 12, "+"));
		}
		buttonList.add(new GuiRangeTriggerInvertRedstone(10, guiLeft + 8,
				guiTop + 62, container.trigger));
	}
	...
	private void renderValue(double value, int x, int y) {
		...
		for (int i = 0; i < 10; i++) {
			...
			fontRendererObj.drawString(str,
					x - 12 * i - fontRendererObj.getCharWidth(str.charAt(0))
							/ 2 + (9 - i + 2) / 3 * 6, y, 0x404040);
			value /= 10;
		}
	...
	protected void actionPerformed(GuiButton button) {
		...
		double initValue = isEnd ? container.trigger.levelEnd
				: container.trigger.levelStart;
		...
		if (newValue != initValue) {
			...
			NuclearNetworkHelper.setRangeTrigger(trigger.xCoord,
					trigger.yCoord, trigger.zCoord, newValue, isEnd);
			if (isEnd)
