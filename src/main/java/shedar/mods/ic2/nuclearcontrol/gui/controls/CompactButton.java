...
public class CompactButton extends GuiButton {
	...
	public CompactButton(int par1, int par2, int par3, int par4, int par5,
			String par6Str) {
	...
	public void drawButton(Minecraft minecraft, int par2, int par3) {
		if (this.visible) {
			...
			boolean var5 = par2 >= this.xPosition && par3 >= this.yPosition
			...
			this.drawTexturedModalRect(this.xPosition, this.yPosition, 0,
					64 + var6 * 12, this.width / 2 + width % 2, this.height);
			this.drawTexturedModalRect(this.xPosition + this.width / 2 + width
					% 2, this.yPosition, 200 - this.width / 2, 64 + var6 * 12,
					this.width / 2, this.height);
			this.mouseDragged(minecraft, par2, par3);
			fontRenderer.drawString(displayString, xPosition
					+ (width - fontRenderer.getStringWidth(displayString)) / 2,
					yPosition + 2, 0x303030);
		}
	}
