...
public class ContainerAverageCounter extends Container {
	...
	public ContainerAverageCounter(EntityPlayer player,
			TileEntityAverageCounter averageCounter) {
		super();
	...
	public void addCraftingToCrafters(ICrafting crafting) {
		...
		NuclearNetworkHelper.sendAverageCounterValue(averageCounter, crafting,
				averageCounter.getClientAverage());
	}
	...
	public void detectAndSendChanges() {
		...
		for (int i = 0; i < crafters.size(); i++) {
			...
			if (lastAverage != average) {
				NuclearNetworkHelper.sendAverageCounterValue(averageCounter,
						crafting, average);
			}
		}
		lastAverage = average;
	...
	public ItemStack transferStackInSlot(EntityPlayer p, int slotId) {
		...
		if (slot != null) {
			...
			if (items != null) {
				...
				if (slotId < averageCounter.getSizeInventory())// moving from
																// counter to
																// inventory
				{
					mergeItemStack(items, averageCounter.getSizeInventory(),
				...
				} else// moving from inventory to counter
				{
					for (int i = 0; i < averageCounter.getSizeInventory(); i++) {
						...
						ItemStack targetStack = averageCounter
								.getStackInSlot(i);
						if (targetStack == null) {
