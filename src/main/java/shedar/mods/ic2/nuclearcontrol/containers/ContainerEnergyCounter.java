...
public class ContainerEnergyCounter extends Container {
	...
	public ContainerEnergyCounter(EntityPlayer player,
			TileEntityEnergyCounter energyCounter) {
		super();
	...
	public void detectAndSendChanges() {
		...
		for (int i = 0; i < crafters.size(); i++) {
			...
			if (lastCounter != counter) {
				NuclearNetworkHelper.sendEnergyCounterValue(energyCounter,
						crafting);
			}
		}
		lastCounter = counter;
	...
	public ItemStack transferStackInSlot(EntityPlayer p, int slotId) {
		...
		if (slot != null) {
			...
			if (items != null) {
				...
				if (slotId < energyCounter.getSizeInventory())// moving from
																// counter to
																// inventory
				{
					mergeItemStack(items, energyCounter.getSizeInventory(),
